...
Plot.plot({
  ...
  marks: [
    ...
    Plot.ruleY([ystarHc], {stroke: "var(--im-fg-primary)", strokeDasharray: "3 3", opacity: 0.5}),
    ...
    Plot.line(pathHc, {x: "t", y: "y", stroke: "var(--im-fg-primary)", strokeWidth: 2.5}),
    ...
    Plot.text([{t: THc * 0.5, y: ystarHc}], {text: d => `y* = ${ystarHc.toFixed(1)}`, dy: -8, fill: "var(--im-fg-primary)"})
  ]
})
```
...
The blue line is physical capital, the red line is human capital, and the ink-black line is output — each converging to its dashed target. Try nudging $s_h$ up by a little and watch *all three* lines lift: more schooling feeds back into more output, which finances more of *both* stocks.
:::
